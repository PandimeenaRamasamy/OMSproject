...
import './style.scss';
import DayAndTime from '../../Component/AddTime'
import vector from '../../Assests/Image/Vector.svg'
import { useDispatch, useSelector } from 'react-redux';
import { PostDeliveryDataRequest } from '../../Redux/Actions';
...
const Delivery = () => {
  ...
  const [timeSlots, setTimeSlots] = useState([{ openingTime: '00:00', closingTime: '00:00' }])
  ...
  const [dayAndTimeComp, setDayAndTimeComp] = useState([<DayAndTime key={0} openingTime={openingTime} closingTime={closingTime} setOpeningTime={setOpeningTime} setClosingTime={setClosingTime}/>])

  //packaging charge
  const [packageCharge ,setPackageCharge] = useState()

  //for schedule delivery
  const [scheduledDay, setScheduledDay] = useState(["EOD", "2D", "3D", "4D", "5D", "6D", "7D"])
  ...
  const addDayAndTime = () => {
    if (timeSlots.length < 3) {
      setTimeSlots([...timeSlots, { openingTime: '00:00', closingTime: '00:00' }])
    }
  }
  ...
  const toggleAll = () => {
    ...
    const handleSaveNextButton = () => {
        ...
        const payloadData = {
            locationId : "715f2abb-d6da-4efe-a6a2-aebdb91e29aa",
            deliverySettingTime,
  ...
  return (
    <div className='delivery'>
      ...
      <div className='button'>
        <input type='submit' className={isEnable ? 'enableBtn' : 'disableBtn'} value="Enable" onClick={enableClick} />
        <input type='submit' className={isEnable ? 'disableBtn' : 'enableBtn'} value="Disable" onClick={disableClick}/>
      </div>
      {showdelivery && <div className='deliveryContent'>
        ...
        <AddTime
            ...
            <p className='deleteTime' onClick={handleDelete}>- Delete Session</p>
      ...
      <div className='schedule'>
        ...
        <div className='radioBtn'>
            ...
            <label>
                <input type="radio" id="yes" name='delivery' value="yes" checked={deliveryOption === 'yes'} onChange={handleScheduleChange} onClick={()=>setShowScheduledDelivery(true)} />
                <p className='sessionName'>Yes</p>
            </label>
            </div>
            <div className='sessionContainer'>
            <label>
                <input type="radio" id="no" name='delivery' value="no" defaultChecked checked={deliveryOption === 'no'} onChange={handleScheduleChange} onClick={()=>setShowScheduledDelivery(false)}/>
                <p className='sessionName'>No</p>
            </label>
            </div>
        </div>
      </div>
      {showScheduledDelivery && <div className='deliveryInfo'>
      ...
      <div className='deliveryOption'>
       ...
       <div className='deliveryOptionContainer'>
        <label>
            <input className='checkbox' value='in house' type='checkbox' onClick={()=>setInHouse(!inHouse)} />
            <p className='cPname'>Inhouse</p>
        </label>
        <label>
            <input className='checkbox' value='3rd party' type='checkbox' onClick={()=>setThirdParty(!thirdParty)}/>
      ...
      {/* Inhouse */}
      {inHouse && <div className='inhouse'>
        <div className='header'>
            <h3>Inhouse</h3>
            <img className={showInHouse ? 'arrowUp' : 'arrowDown'} onClick={()=>setShowInHouse(!showInHouse)} src={vector} />
        </div>
       {showInHouse && <div className='body'>
            ...
            <div className='feesStructure'>
                ...
                <div className='radioBtn'>
                    <div className='sessionContainer'>
                        <label>
                            <input type="radio" id="based on distance" name='feesStructure' value="based on distance" 
                                onClick={() => 
                                {
                                    setIsBOD(true) 
                                    setIsFlatFee(false)
                        ...
                        <div className='sessionContainer'>
                        <label>
                            <input type="radio" id="flat fee" name='feesStructure' value="flat fee" 
                                onClick={() => 
                                {
                                    setIsFlatFee(true) 
                                    setIsBOD(false)
                ...
                {isBOD && <div className='basedOnDistance'>
                ...
                {(isFlatFee && !isBOD) && <div className='flatfee'>
                    <h3>Flat Fee</h3>
      ...
      {thirdParty && 
        <div className='thirdParty'>
            <div className='header'>
                <h3>3rd Party</h3>
                <img  className={showThirdParty ? 'arrowUp' : 'arrowDown'} onClick={()=>setShowTHirdParty(!showThirdParty)} src={vector} />
            </div>
            {showThirdParty && <div className='body'>
            ...
            <label>
                <input 
                    ...
                    onClick={() => handleCheckboxChanges('doordash', setShowDoorDash)}
                />
            ...
            <label>
                <input 
                    ...
                    type='checkbox' 
                    checked={showDunzo}
                    onClick={() => handleCheckboxChanges('dunzo', setShowDunzo)}
                />
            ...
            <label>
                <input 
                    ...
                    onClick={() => handleCheckboxChanges('uberEats', setShowUberEats)}
                />
                ...
                <div className='thirdPartyInputContaianer'>
                    {showDoorDash && <div className='thirdPartyInput'>
                        <p>Doordash ID</p>
                        <input 
                            type='text' 
                            value={doorDash}
                            onChange={(e) => {setDoorDash(e.target.value)}}
                        />
                    </div>}
                    {showDunzo && <div className='thirdPartyInput'>
                        <p>Dunzo ID</p>
                        <input 
                            type='text' 
                            value={dunzo}
                            onChange={(e) => {setDunzo(e.target.value)}}
                        />
                    </div>}
                    {showUberEats && <div className='thirdPartyInput'>
                        <p>UberEats ID</p>
                        <input 
                            type='text' 
                            value={uberEats}
    ...
    <div className='button'>
      <div className="button buttonsContainer">
          <button className="button cancelBtn" onClick={handleClearAllButton}>Clear All</button>
          <button className="button saveBtn" onClick={handleSaveNextButton}>Save & Next</button>
      </div>
    </div>
    </div>
  )
